package src._6_topo_sort._1_basics;
...
public class TopologicalSort_DFS {
    ...
    public static void main(String[] args) {
        ...
        TopologicalSort_DFS obj = new TopologicalSort_DFS();
    ...
    public ArrayList<Integer> topoSort(int v, int[][] edges) {
        ...
        List<List<Integer>> adjList = getAdjList(v, edges, true); // adj list from edges
        Stack<Integer> stack = new Stack<>(); // storing topo sort output
        boolean[] isVisited = new boolean[v];

        // there can be multiple components, use for loop to visit all the components
        for (int i = 0; i < v; i++) {
            if (!isVisited[i]) {
        ...
        // topo sort is stored in stack, return it in list form
    ...
    private void dfs(int currNode, List<List<Integer>> adjList, boolean[] isVisited, Stack<Integer> stack) {
        ...
        stack.push(currNode); // all the adj nodes are visited, safe to add to stack.
    }
